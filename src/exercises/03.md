...
-   Here we see an example of `React.memo()` vs `React.useMemo()`
    -   `React.memo()` is used for components (works like an HOC), `useMemo()` is not
    -   `const EnhancedComponent = React.memo(Component[, compareProps])` where by default, React will shallow-compare the props to determine if it should re-render the Component, but you can provide your own `function(prevProps, nextProps) => boolean` if you want to customize the comparison
-   It's a mistake to _blindly_ apply `React.memo()` all over the place thinking it'll optimize your app; it _can_ make your app faster, but it can make it a lot more complex as well because the memoization ends-up having very long tenticles...
-   Performance optimizations almost always come with a complexity cost, so make sure the added complexity is worth it
-   You can memoize a third-party component by wrapping it yourself into `React.memo()`
